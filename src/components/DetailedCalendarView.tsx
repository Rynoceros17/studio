...
const getTaskStyle = (task: Task, colorToApply: string | null | undefined): React.CSSProperties => {
  ...
  if (visualDuration <= 0) {
      ...
      if (originalDuration > 0) {
        const top = (startTotalMinutes / 15) * 0.611296875;
        const height = (originalDuration / 15) * 0.611296875;
        return {
          top: `${top}rem`,
          height: `${height}rem`,
  ...
  const top = (visualStartMinutes / 15) * 0.611296875;
  const height = (visualDuration / 15) * 0.611296875;
...
export function DetailedCalendarView({ tasks, onCreateTask, onEditTask, onDeleteTask, onToggleComplete, completedTasks, updateTask }: DetailedCalendarViewProps) {
  ...
  useEffect(() => {
    if (scrollContainerRef.current) {
      const sevenAmHourSlotPosition = 7 * (2.4451875 * 16); // 7 * (0.611296875rem * 4 slots) * 16px/rem
    ...
    <div className="flex flex-col h-full" onMouseUp={handleMouseUp} onMouseLeave={isSelecting ? handleMouseUp : undefined}>
      ...
      <div ref={scrollContainerRef} className="flex flex-grow overflow-auto">
        <div className="w-14 text-[10px] text-center shrink-0">
          ...
          {timeSlots.map(time => <div key={time} className="h-[2.4451875rem] relative text-muted-foreground"><span className="absolute -top-2 left-1/2 -translate-x-1/2 bg-background px-1 z-10">{time}</span></div>)}
        </div>
              ...
              <div key={dateStr} className={cn("relative border-l", isToday && "bg-secondary/20")}>
                ...
                <div className="relative">
                  {timeSlots.map((_, hour) => (
                    <div key={hour} className="h-[2.4451875rem] border-t relative">
                      {Array.from({ length: 4 }).map((__, quarter) => {
                        const cellId = getCellId(dayIndex, hour, quarter);
                        return (
                          <div
                            key={quarter}
                            className={cn("h-[0.611296875rem]", quarter === 3 ? "border-b border-solid border-border" : "border-b border-dashed border-border/40", isCellSelected(dayIndex, hour, quarter) && "bg-primary/30")}
                            data-cell-id={cellId}
